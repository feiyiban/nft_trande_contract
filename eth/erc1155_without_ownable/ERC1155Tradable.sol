 ...
 */
contract ERC1155Tradable is ContextMixin, ERC1155, NativeMetaTransaction {
  ...
  /**
    ...
    * @param _tokenUri Optional URI for this token type
  ...
  /**
    * @dev createBatch tokens for each id in _ids
    * @param _initialOwner          The address to mint tokens to
    * @param _ids The ids Array of the token to create (must not currenty exist).
    * @param _initialSupply         Array of amounts of tokens to mint per id
    * @param _tokenUris             Array of tokenUri of tokens to mint per id
    * @param _data                  Data to pass if receiver is contract
    */
  function createBatch(
    ...
    bytes memory _data
  ) public  returns (uint256[] memory _mintTokenIds) {

    require(_ids.length > 0,"The _ids must be have");
    require(_initialSupply.length > 0,"The _amounts must be have");
    require(_tokenUris.length > 0,"The _tokenUrls must be have");
    require(_ids.length == _initialSupply.length,"The _ids length must be Equal _initialSupply length");
    require(_ids.length == _tokenUris.length,"The _ids length must be Equal _tokenUris length");
    require(_initialSupply.length == _tokenUris.length,"The _initialSupply length must be Equal _tokenUris length");

    uint256 newTokenId;
    uint256[] memory returnTokenIds = new uint256[](_initialSupply.length);

    for(uint256 i = 0; i < _initialSupply.length; i++){
            newTokenId = _ids[i];
            customUri[newTokenId] = _tokenUris[i];
            creators[newTokenId] = _msgSender();
            returnTokenIds[i] = newTokenId;
        }

    _mintBatch(_initialOwner, returnTokenIds, _initialSupply, _data);
