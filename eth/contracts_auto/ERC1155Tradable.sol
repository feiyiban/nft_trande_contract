...
contract ERC1155Tradable is ContextMixin, ERC1155, NativeMetaTransaction, Ownable {
  ...
  Counters.Counter private _autoTokenId;
  ...
  function create(
    ...
    require(bytes(_tokenUri).length > 0,"The _uri must be have");
    require(_initialSupply > 0,"The _initialSupply must be have");

    _autoTokenId.increment();
    uint256 _id = _autoTokenId.current();
  ...
  function createBatch(
    ...
    require(_initialSupply.length == _tokenUris.length,"The _tokenUrl length must be Equal _amounts length");
    ...
    for(uint256 i = 0; i < _initialSupply.length; i++){
            _autoTokenId.increment();
            newTokenId = _autoTokenId.current();
